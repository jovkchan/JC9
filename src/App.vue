...
onMounted(async () => {
  ...
  const launchedBefore = localStorage.getItem('jc9_launched')

  if (win.label === 'splash') {
    isSplash.value = true
    if (launchedBefore) {
      // Not first launch — skip splash, show main immediately
      const mainWin = await WebviewWindow.getByLabel('main')
      if (mainWin) { await mainWin.show(); await mainWin.setFocus() }
      win.close()
      return
    }
    // First launch — load data, skip initListeners (splash has no PTY)
    await store.loadProjects()
    localStorage.setItem('jc9_launched', '1')
    const mainWin = await WebviewWindow.getByLabel('main')
    if (mainWin) { await mainWin.show(); await mainWin.setFocus() }
...
<template>
  ...
  <div v-if="isSplash" class="splash">
    <svg class="splash-icon" viewBox="0 0 800 800" width="150" height="150" fill="currentColor">
...
.splash { position:fixed; inset:0; display:flex; align-items:center; justify-content:center; background:transparent; }
.splash-icon { color:var(--jc-color-accent,#8a58ff); animation:flip 1s linear infinite; }
