      ...
      <main class="settings-content">
        ...
        <div v-if="activeTab === 'general'" class="settings-pane">
          ...
          <div class="form-group">
            <label>默认笔记格式</label>
            <JcSelect beam glow :model-value="defaultFormat" :options="formatOptions" style="width: 100%" @update:model-value="(v) => defaultFormat = v as 'markdown' | 'plain'" />
            <span class="help-text">新建备忘时的默认输入解析格式</span>
          </div>
          <div class="form-group">
            <label>新建笔记默认可见性</label>
            <JcSelect beam glow :model-value="defaultVisibility" :options="visibilityOptions" style="width: 100%" @update:model-value="(v) => defaultVisibility = v as 'PRIVATE' | 'PUBLIC'" />
        ...
        <div v-if="activeTab === 'ai'" class="settings-pane">
          ...
          <JcModal v-model:open="showModelForm" :title="(newModelForm.id ? '编辑' : '添加') + '模型配置'" width="480">
              <div class="form-group"><label>配置名称</label><JcInput beam glow v-model="newModelForm.name" placeholder="例如：DeepSeek 主力" /></div>
              <div class="form-group">
                <label>供应商</label>
                <JcSelect beam glow :model-value="newModelForm.provider" :options="providerOptions" style="width: 100%" @update:model-value="(v) => { newModelForm.provider = v as 'ollama' | 'deepseek' | 'openai' | 'gemini' | 'vllm'; setProviderDefaults() }" />
              </div>
              <div class="form-group"><label>Endpoint</label><JcInput beam glow v-model="newModelForm.endpoint" @blur="onEndpointBlur" /></div>
              <div class="form-group" v-if="newModelForm.provider !== 'ollama' && newModelForm.provider !== 'vllm'"><label>API Key</label><JcInput beam glow v-model="newModelForm.apiKey" type="password" placeholder="sk-..." /></div>
              <div class="form-group">
                <label>Model</label>
                ...
                <JcInput beam glow v-else v-model="newModelForm.model" placeholder="多个用英文逗号分隔" />
              </div>
              <div class="form-row">
                <div class="form-group form-half"><label>输入价格 (元/百万)</label><JcInputNumber :model-value="newModelForm.inputPrice" :min="0" :step="0.1" size="small" beam glow @update:model-value="newModelForm.inputPrice = $event ?? 0" /></div>
                <div class="form-group form-half"><label>输出价格 (元/百万)</label><JcInputNumber :model-value="newModelForm.outputPrice" :min="0" :step="0.1" size="small" beam glow @update:model-value="newModelForm.outputPrice = $event ?? 0" /></div>
              </div>
              <div class="form-group"><label>熔断限额 (元)</label><JcInputNumber :model-value="newModelForm.costLimit" :min="0" :step="0.5" size="small" beam glow @update:model-value="newModelForm.costLimit = $event ?? 0" /></div>
              <template #footer>
        ...
        <div v-if="activeTab === 'ai-roles'" class="settings-pane">
          ...
          <JcModal v-model:open="showRoleForm" :title="(newRoleForm.id ? '编辑' : '添加') + ' AI 角色'" width="440">
              <div class="form-group"><label>角色名称</label><JcInput beam glow v-model="newRoleForm.name" placeholder="例如：测试工程师" /></div>
              <div class="form-group"><label>角色介绍</label><JcInput beam glow v-model="newRoleForm.description" placeholder="简述该角色的核心职责" /></div>
              <div class="form-group"><label>专属系统提示词</label><JcTextarea v-model="newRoleForm.systemPrompt" beam glow :beam-size-ratio="0.6" :rows="6" /></div>
              <template #footer>
        ...
        <div v-if="activeTab === 'skills'" class="settings-pane">
          <div class="skills-toolbar">
            <JcInput beam glow v-model="skillsSearch" placeholder="搜索技能..." style="flex: 1; min-width: 0" />
        ...
        <div v-if="activeTab === 'mcp'" class="settings-pane">
          ...
          <div class="builtin-mcp-section">
            ...
            <div class="mcp-server-compact">
              ...
              <div class="mcp-compact-row">
                ...
                <JcInputNumber :model-value="mcpPortInput ? Number(mcpPortInput) : null" :min="1024" :max="65535" size="small" beam glow @update:model-value="mcpPortInput = $event != null ? String($event) : ''" style="width: 110px" />
              </div>
              ...
              <div v-if="showApiKeyForm" style="border-top:1px solid var(--jc-border-default);padding-top:10px;margin-top:4px">
                ...
                <div class="form-group">
                  ...
                  <div style="display:flex;gap:4px">
                    <JcInput beam glow v-model="newApiKey.key" style="flex:1;min-width:0" :placeholder="editingApiKeyId ? '' : '自动生成'" readonly />
                    <button class="btn-sm" @click="regenerateKeyInForm">生成</button>
                  </div>
                </div>
                <div class="form-group">
                  <label>备注</label>
                  <JcInput beam glow v-model="newApiKey.label" placeholder="如：JCGO 项目" />
                </div>
                <div class="form-group">
                  <label>Scope（项目标识）</label>
                  <JcInput beam glow v-model="newApiKey.scope" placeholder="如：JCGO" />
                </div>
                <div class="form-group">
                  <label>白名单分组</label>
          ...
          <div class="builtin-mcp-section" style="margin-top:8px">
            ...
            <div class="mcp-compact-row">
              ...
              <span class="mcp-compact-sep">|</span>
              <span class="mcp-compact-label">端口</span>
              <JcInputNumber :model-value="noteSharePortInput ? Number(noteSharePortInput) : null" :min="1024" :max="65535" size="small" beam glow @update:model-value="noteSharePortInput = $event != null ? String($event) : ''" style="width: 110px" />
              <span style="flex:1"></span>
          ...
          <template v-if="mcpViewMode === 'list'">
            ...
            <JcModal v-model:open="showMcpForm" title="连接 MCP 服务器" width="440">
                <div class="form-group"><label>方式</label><JcSelect beam glow :model-value="mcpForm.transport" :options="mcpTransportOptions" style="width: 100%" @update:model-value="(v) => mcpForm.transport = v as 'sse' | 'stdio'" /></div>
                <div class="form-group"><label>名称</label><JcInput beam glow v-model="mcpForm.name" placeholder="my-server" /></div>
                <template v-if="mcpForm.transport === 'sse'"><div class="form-group"><label>SSE URL</label><JcInput beam glow v-model="mcpForm.url" placeholder="https://example.com/mcp" /></div></template>
                <template v-if="mcpForm.transport === 'stdio'">
                  <div class="form-group"><label>命令</label><JcInput beam glow v-model="mcpForm.command" placeholder="npx" /></div>
                  <div class="form-group"><label>参数 (逗号分隔)</label><JcInput beam glow v-model="mcpForm.argsText" placeholder="arg1, arg2" /></div>
                </template>
                <template #footer>
        ...
        <div v-if="activeTab === 'memory'" class="settings-pane" style="display:flex;flex-direction:column;overflow:hidden;height:100%">
          ...
          <div style="flex-shrink:0">
            ...
            <div style="display:flex;gap:6px;margin-top:8px">
              <JcInput beam glow v-model="memorySearch" placeholder="搜索标题或内容..." style="flex:1;min-width:0" @keyup.enter="searchMemory" />
            ...
            <template #title>
              ...
              <template v-else>
                <div class="memory-form">
                  <JcInput beam glow v-model="memoryForm.title" placeholder="记忆标题" />
                  <div style="display:flex;gap:6px;margin-top:6px">
                    <JcInput beam glow v-model="memoryForm.scope" placeholder="scope（项目标识）" style="flex:1;min-width:0" />
                    <JcSelect beam glow v-model="memoryForm.type" :options="memoryTypeOptions" style="flex:1;min-width:0" />
                  </div>
                  <JcInput beam glow v-model="memoryForm.topicKey" placeholder="topic_key（去重用）" style="margin-top:6px" />
                  <JcTextarea v-model="memoryForm.content" beam glow :beam-size-ratio="0.6" :rows="10" placeholder="**What**: 做了什么
**Why**: 为什么
**Where**: 涉及文件
