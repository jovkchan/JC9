    ...
    <div v-for="(t,i) in store.memoryTabs" :key="'memc'+t.id" class="content" v-show="showMemoryTabs&&store.activeTabType==='memory'&&i===store.activeMemoryIndex">
      ...
      <div v-else class="memory-edit-view">
        <div class="mem-edit-field">
          <label>标题</label>
          <JcInput beam glow v-model="t.title" />
        </div>
        <div class="mem-edit-field">
          <label>类型</label>
          <JcSelect beam glow :model-value="t.type" :options="memTypeOptions" style="width: 100%" @update:model-value="(v) => t.type = v as string" />
        </div>
        <div class="mem-edit-field">
          <label>Scope</label>
          <JcInput beam glow v-model="t.scope" placeholder="项目标识" />
        </div>
        <div class="mem-edit-field">
          <label>Topic Key</label>
          <JcInput beam glow v-model="t.topicKey" placeholder="去重键" />
        </div>
        <div class="mem-edit-field content-field">
          <label>内容</label>
          <JcTextarea v-model="t.content" beam glow :beam-size-ratio="0.6" class="jc-fill" />
        </div>
      </div>
    </div>
