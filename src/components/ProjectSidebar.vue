  ...
  <aside class="side" :class="{ 'no-tabs': !!forcedTab }">
    ...
    <div v-show="effectiveTab==='projects'" class="panel">
      ...
      <div v-if="showAdd" class="add-panel">
        <JcInput beam glow v-model="newName" placeholder="项目名称" @keyup.enter="handleAdd" />
        <div class="row"><JcInput beam glow v-model="newDir" placeholder="项目目录" style="flex:1;min-width:0" @keyup.enter="handleAdd" /><JcButton @click="pickDir">...</JcButton></div>
        ...
        <div v-for="p in store.projects" :key="p.id">
          <div class="proj" :class="{sel:store.selectedProjectId===p.id}" @click="toggleExpand(p.id);store.selectedProjectId=p.id" @contextmenu="openProjCtx($event,p.id)">
            <template v-if="editingProjId===p.id">
              <JcInput beam glow class="proj-edit-input" v-model="editProjName" @keyup.enter="confirmRenameProj" @keyup.escape="editingProjId=''" @blur="confirmRenameProj" @click.stop />
            </template>
            <template v-else>
          ...
          <div v-if="expandedProjects.has(p.id)" class="cmds">
            <div v-for="cmd in p.commands" :key="cmd.id" class="cmd" :class="{on:isRunning(p.id,cmd.id)}" @contextmenu="openCmdCtx($event,p.id,cmd)">
              <template v-if="editingCmdId===p.id+'::'+cmd.id">
                <JcInput beam glow class="cmd-edit-input" v-model="editCmdName" @keyup.enter="confirmRenameCmd" @keyup.escape="editingCmdId=''" @blur="confirmRenameCmd" @click.stop />
              </template>
              <template v-else>
    ...
    <div v-show="effectiveTab==='tools'" class="panel" style="display:flex;flex-direction:column">
      ...
      <div class="search-bar">
        <JcInput beam glow v-model="toolSearchQuery" placeholder="搜索实用工具..." />
      </div>
